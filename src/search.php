...
function printSearchMessages($msgs,$mailbox, $cnt, $imapConnection, $where, $what, $usecache = false, $newsort = false) {
    ...
    if ($cnt > 0) {
        ...
        echo '<form name="' . $form_name . '" method="post" action="move_messages.php">' ."\n" .
             ...
             '<input type="hidden" name="flagSearch" value="true">';
             addHidden('smtoken', sm_generate_security_token()) . "\n";
             
        if(isset($_GET['what']))
            echo '<input type="hidden" name="whatHidden" value="' .  $_GET['what'] . '">';

        echo '<tr><td height="5" bgcolor="'.$color[4].'"></td></tr>';
        echo '<tr><td>';
        echo '    <table class="tableMail" width="100%" cellpadding="1" cellspacing="0" align="center"'.' border="0" bgcolor="'.$color[9].'">';
        echo '     <tr><td>';
        echo '       <table class="gridmail" width="100%" cellpadding="1" cellspacing="0" align="center" border="0" bgcolor="'.$color[5].'">';
        echo '<tr><td>';
       
        displayMessageArray($imapConnection, $cnt, 1,
            $msort, $mailbox, $sort, '', $cnt, $where, $what);
...
/*  search one folder option  */
    ...
    if (($submit == _("Search") || $submit == 'Search_no_update') && !empty($what)) {
        ...
        if (count($msgs)) {
            global $show_num;
            if(isIE())
                printSearchMessages($msgs, $mailbox,100, $imapConnection,$where, $what, false, false);
            else
                printSearchMessages($msgs, $mailbox,200, $imapConnection,$where, $what, false, false);
        } else {
            echo '<br /><center>' . _("No Messages Found") . '</center>';
        }
    }
