...
$td_str = '<form name="f"  id="identities" action="options_identities.php" method="post"><br />' . "\n"
...
<script>
    ...
    function showpA(){
        $(".msgUpdateIdent").show();
        setTimeout('$(".msgUpdateIdent").slideUp("normal")',3500);
    }
    function showpU(){
        $(".msgUpdateIdent").hide();
    }
...
<div class="msgUpdateIdent"><?php echo _("Updated");?></div>
<?php
echo  html_tag('table', "\n" .
        html_tag('tr', "\n" .
            html_tag('td', "\n" .
...
function ShowIdentityInfo($title, $identity, $id ) {
    ...
    $return_str .= '</tr>' . "\n";
    $return_str .= sti_input( _("Full Name") , sprintf($name, $id, 'full_name'), utf8_decode($identity['full_name']), $bg);
    ...
    $return_str .= '  <td> &nbsp; </td>' . "\n";
    $return_str .= '  <td>' . "\n";
    $return_str .= '    <input type="submit" class="btnIden" name="smaction[save][' . $id . ']" value="' . _("Save / Update") . '" />' . "\n";

    if (!$empty && $id > 0) {
        $return_str .= '    <input type="submit" class="btnIden" name="smaction[makedefault][' . $id . ']" value="' . _("Make Default") . '" />' . "\n";
        $return_str .= '    <input type="submit" class="btnIden" name="smaction[delete]['.$id.']" value="' . _("Delete") . '" />' . "\n";

        if ($id > 1) {
            $return_str .= '    <input type="submit" class="btnIden" name="smaction[move]['.$id.']" value="' . _("Move Up") . '" />' . "\n";
        }
