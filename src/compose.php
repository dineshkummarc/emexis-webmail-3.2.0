    ...
    if ($passed_id) {
        ...
        if (count($idents) > 1) {
            foreach($idents as $nr=>$data) {
                $enc_from_name = '"'. utf8_decode($data['full_name']).'" <'. $data['email_address'].'>';
...
function showInputForm ($session, $values=false) {
    ...
    <script>initEditor('.textareaCompose');</script>
   ...
   // }
    ...
    echo addHidden('composesession', $composesession).
        ...
        ' <div class="textAttach"><input name="attachfile" size="' . (isIE()?'32':'20')  .'" type="file" />' . "\n" .
		...
		"aoColumns": [
            ...
            $('a.address,a.addressEn').click(
                function(){
                        ...
                        function(){
                            if($('textarea:visible')){
                                if($('textarea:visible').is(':empty'))
                                    $('textarea:visible').text($(this).text());
                                else
                                    $('textarea:visible').text($('textarea:visible').text() + ', ' + $(this).text());
                            }
                        }
                    )
                    .css("cursor","pointer");
                    ...
                    $('#actionToField').click(
                        function(){                            
                            if(!$("#toFieldTo").is(":empty")){                                
                                if($("input[name=send_to]").val() == ""){                                    
                                    $("input[name=send_to]").val($("#toFieldTo").text())
                                }else{                                    
                                    $("input[name=send_to]").val($("input[name=send_to]").val() + ',' + $("#toFieldTo").text());
                                }
                            }
                            if(!$("#toFieldCC").is(":empty")){
                                if($("input[name=send_to_cc]").val() == ""){
                                    $("input[name=send_to_cc]").val($("#toFieldCC").text())
                                }else{
                                    $("input[name=send_to_cc]").val($("input[name=send_to_cc]").val() + ',' + $("#toFieldCC").text());
                                }
                            }                            
                            if(!$("#toFieldBCC").is(":empty")){
                                if(!$(".rowBcc").children().children().is(":visible"))
                                    $(".rowBcc").children().children().slideToggle('fast');
                                if(!$("#toFieldBCC").is(":empty")){
                                    if($("input[name=send_to_bcc]").val() == ""){
                                        $("input[name=send_to_bcc]").val($("#toFieldBCC").text())
                                    }else{
                                        $("input[name=send_to_bcc]").val($("input[name=send_to_bcc]").val() + ',' + $("#toFieldBCC").text());
                                    }
                                }
                            }
