...
echo '<div id="contentOptions">';
echo html_tag( 'table', '', 'left', 'transparent', 'width="95%" cellpadding="1" cellspacing="0" border="0"' ) . "\n" .
        html_tag( 'tr' ) . "\n" .
            html_tag( 'td', '', 'left' ) .
...
if ($optpage == SMOPT_PAGE_MAIN) {
    ...
    if ($optmode == SMOPT_MODE_SUBMIT) {
        ...
        if (isset($optpage_save_error) && $optpage_save_error!=array()) {
            ...
            echo 'Successfully Saved Options';
        }
...
<span class="saveSuccess"><?php echo _("Successfully Saved Options");?></span>
<span class="saveError"><?php echo _("Error(s) occurred while saving your options");?></span>
<?php
global $dsn_pear;
if(returnCountEmailOptions($dsn_pear,$username) == 0){
?>
<script>
    $(function(){
        $('[name=new_email_address]').addClass('fieldRequired').focus();
        $('[name=new_email_address]').attr('title','<?php echo _("Required field");?>');
        //$('<span style="color:red">*</span>').insertAfter('input.fieldRequired');
    });
</script>
<?php
}
