...
function formatEnvheader($mailbox, $passed_id, $passed_ent_id, $message,
    ...
    if ($default_use_mdn) {
     ...
     $s .= '<div class="formatHeader"><table width="100%">';
     foreach ($env as $key => $val) {
        if ($val) {
            $style = "";
            if($key == _("Subject"))
                $style = "class='subjectMsg' onclick='showInfoMsg(this)'";
            $s .=  "<tr $style class='row'>"
            . '<td >'.  (($key == _("Subject"))?'&nbsp<span class="hideSub" >+</span>&nbsp;&nbsp;':'')
            . '<span>' . $key  . '</span>:&nbsp;&nbsp;'
            . $val  . '</td>'
            . '</tr>';
        }
    ...
    $s .= '</table></div>';
    echo '<input type="hidden" name="subjectLabel" value="'. $env[_("Subject")] . '">';
    ...
    echo '<tr><td height="0" colspan="2" bgcolor="'.
...
function formatMenubar($mailbox, $passed_id, $passed_ent_id, $message, $mbx_response) {
    ...
    $s = '<table width="100%" cellpadding="3" cellspacing="0" align="center"'.
         ' border="0" class="MenuReadWin"><tr>' .
         html_tag( 'td', '', 'left', '', 'width="50%"' ) . '<small>';
    ...
    //6FfC1zWprHMQ INBOX INBOX unread 3165 1 undefined

    $unread_url = 'javascript:markUnread("' . sm_generate_security_token() . '","' . $urlMailbox .'","'
    . $urlMailbox .'","unread","' . $passed_id . '","' . $startMessage . '","");';
    ...
    if (!(isset($passed_ent_id) && $passed_ent_id)) {
        ...
        $unread_url = 'javascript:markUnread("' . sm_generate_security_token() . '","' . $urlMailbox .'","'
        . $urlMailbox .'","unread","' . $passed_id . '","' . $startMessage . '","");';
        $s .= '<a href=' . $unread_url . '>' . _("Mark as unread") . '</a>';
        ...
        $delete_url = 'javascript:deleteMsg("' . sm_generate_security_token() . '","' . $urlMailbox .'","'
        . $urlMailbox .'","delete","","' . $passed_id . '","'. $startMessage . '","");';
    ...
    if (($mailbox == $draft_folder) && ($save_as_draft)) {
        ...
        $s .= "<a href=javascript:openCompose($passed_id,'$urlMailbox',$startMessage,0,'draft')>" . _("Resume Draft") . '</a>';
    ...
    if (isset($comp_alt_uri)) {
        $s .= $topbar_delimiter;
        $s .= "<a href=javascript:openCompose($passed_id,'$urlMailbox',$startMessage,0,'edit_as_new')>" . _("Edit Message as New") . '</a>';

        
    }

 
    html_tag( 'td', '', 'right', '', 'width="50%" nowrap' ) . '<small>';
    ...
    //. $urlMailbox .'","delete","","' . $passed_id . '","'. $startMessage
    $s .= $topbar_delimiter;
    $s .= "<a href=javascript:openCompose($passed_id,'$urlMailbox',$startMessage,0,'forward')>" . _("Forward") . '</a>';    

    if ($enable_forward_as_attachment) {
        $comp_action_uri = $comp_uri . '&amp;smaction=forward_as_attachment';
        $s .= $topbar_delimiter;
        $s .= "<a href=javascript:openCompose($passed_id,'$urlMailbox',$startMessage,0,'forward_as_attachment')>" . _("Forward as Attachment") . '</a>';
        //$s .= makeComposeLink($comp_action_uri, _("Forward as Attachment"));
    }

    $comp_action_uri = $comp_uri . '&amp;smaction=reply';
    $s .= $topbar_delimiter;
    $s .= "<a href=javascript:openCompose($passed_id,'$urlMailbox',$startMessage,0,'reply')>" . _("Reply") . '</a>';    

    $comp_action_uri = $comp_uri . '&amp;smaction=reply_all';
    $s .= $topbar_delimiter;
    $s .= "<a href=javascript:openCompose($passed_id,'$urlMailbox',$startMessage,0,'reply_all')>" . _("Reply All") . '</a>';
...
echo '  <tr><td>';
echo '    <table width="100%" cellpadding="0" cellspacing="0" align="center" border="0" bgcolor="'.$color[9].'">';
echo '      <tr><td>';
echo '        <table width="100%" cellpadding="0" cellspacing="0" align="center" border="0">';
echo '          <tr bgcolor="'.$color[4].'"><td>';
// echo '            <table cellpadding="1" cellspacing="5" align="left" border="0">';
echo html_tag( 'table' ,'' , 'left', '', 'cellpadding="0" cellspacing="0" border="0"' );
...
if ($attachmentsdisplay) {
   echo '  <tr><td>';
   echo '    <table width="100%" cellpadding="1" cellspacing="0" align="center"'.' border="0"  class="titleAttachRead"';
   echo '     <tr><td>';
   echo '       <table width="100%" cellpadding="0" cellspacing="0" align="center" border="0" class="lineAttach">';
   echo '        <tr><td class="titleAttachRead">';
   echo '           <b>' . _("Attachments") . ':</b>';
   echo '        </td></tr>';
   echo '        <tr><td>';
   echo '          <table width="100%" cellpadding="2" cellspacing="2" align="center"'.' border="0" class="lineAttach"><tr><td>';
...
echo '</table>';
echo '<input id="idMsgRead" type="hidden" value="' . $passed_id . '">';
