    ...
    foreach ($option_groups as $next_optgrp) {
        ...
        if ($next_optgrp['name'] != '') {
            echo html_tag( 'tr', "\n".
                        ...
                        'left' ,'', 'valign="middle" colspan="2" nowrap' )
                    ) ."\n";
        }
        ...
        foreach ($next_optgrp['options'] as $option) {
            if ($option->type != SMOPT_TYPE_HIDDEN) {
                ...
                if ($option->type == SMOPT_TYPE_INFO)
                    echo html_tag('tr', "\n" . html_tag('td', "\n" . html_tag('table', "\n" . html_tag('tr', "\n" . html_tag('td', "\n" . $option->createHTMLWidget())), '', $color[$info_bgcolor], 'width="' . $info_width . '%"'), 'left' ,'', 'colspan="2" valign="middle"')) ."\n";
                else
                    echo html_tag( 'tr', "\n".
                               html_tag( 'td', $option->caption . (!empty($option->caption) ? ':' : ''), 'left' ,'', 'valign="middle"' . ($option->caption_wrap ? '' : ' style="white-space:nowrap"') ) .
