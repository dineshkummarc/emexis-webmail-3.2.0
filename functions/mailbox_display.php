...
function printMessageInfo($imapConnection, $t, $not_last=true, $key, $mailbox,
    ...
    if (!isset($msg['FLAG_SEEN']) || ($msg['FLAG_SEEN'] == false)) {
        ...
        /*if($msg["RECEIVED_TIME_STAMP"] > (time() - $minutes * 60)){
            ...
            echo "<script>popupNotification('" . $subject ."','{$msg['FROM-SORT']}','{$msg['ID']}');</script>";
        }
        echo '<pre>';
        print_r($_SESSION);
        echo '<pre>;';
   ...
   // echo "<td bgcolor='$hlt_color'" . ((is_vertical())?"rowspan=2":"") . "' VALIGN='MIDDLE'  class='move'><div>x</div></td>";
    //echo $hlt_color;
...
function printHeader($mailbox, $sort, $color, $showsort=true) {
    ...
    echo '<th><div style="width:5%">&nbsp;</div></td>'  ;
    $i = 0;
...
function get_paginator_link($box, $start_msg, $use, $text) {
    ...
    $result = "<a href=\"javascript:loadListMails('$layout',$('input[name=mailbox]:first').val(),0,$use,1,6, $start_msg)\">$text</a>";
    return ($result);
/*
    if (preg_match('/^(.+)\?.+$/',$PHP_SELF,$regs)) {
        $source_url = $regs[1];
