...
function formatAttachments($message, $exclude_id, $mailbox, $id) {
    ...
    foreach ($att_ar as $att) {
        ...
        $attachments .= '<tr><td>' 
             . decodeHeader($display_filename).'&nbsp;</td>' .
        ...
        foreach ($links as $val) {
            ...
            if(strcmp($val['text'],_("Download")) == 0)
                $attachments .= '<a href="' . $val['href'] . '">' .  $val['text'] . '</a>';
            else{
                if(strcmp(htmlspecialchars($type1),'calendar') == 0){
                    $link = $val['href'];
                    $new_link = str_replace('../src/view_text.php', '../calendar/backend/parse_ics_mail.php', $link);
                    $attachments .= '<a href=javascript:dialogImgTexMail("' . $val['href']. '")>' .  $val['text'] . '</a>';
                    $attachments .=  '&nbsp;&nbsp;|&nbsp;&nbsp;' . '<a href=javascript:dialogConfirmCalendar("' . $new_link . '")>'
                    . ('Add calendar') . '</a>';
                }else{
