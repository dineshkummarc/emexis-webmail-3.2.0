...
function sqimap_mailbox_list($imap_stream, $force=false) {
    ...
    if (!sqgetGlobalVar('boxesnew',$boxesnew,SQ_SESSION) || $force) {
        ...
        $read = sqimap_run_command ($imap_stream, "LIST \"\" \"*\"",true, $response, $message);
        ...
        for ($i=0; $i < count($sorted_lsub_ary); $i++) {
            ...
            if (isset($read[$i][1]) && substr($read[$i][1],-3) == "}\r\n") {
                if (preg_match('/^(\* [A-Z]+.*)\{[0-9]+\}([ \n\r\t]*)$/',
                     $read[$i][0], $regs)) {
                    $read[$i][0] = $regs[1] . '"' . addslashes(trim($read[$i][1])) . '"' . $regs[2];
                }
            }

            if (isset($read[$i][0])) {
                $sorted_list_ary[$i] = $read[$i][0];
            } else {
                $sorted_list_ary[$i] = '';
            }
        }
