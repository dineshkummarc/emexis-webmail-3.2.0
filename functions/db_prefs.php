...
if(verifiedClass("DB.php")){
    ...
    if(!include_once(SM_PATH . 'pear/DB.php')){
...
class dbPrefs {
    ...
    function open() {
      ...
      /*
       * Seleciona as preferências do default pref do banco e seta para todos os usuários
       */
     function selectDefault() {
        ...
        global $username;
        $queryFlag = sprintf("SELECT %s FROM %s ".
                         "WHERE %s = 'updatepref' and %s = '$username' ",
                         $this->val_field,
                         $this->table,
                         $this->key_field,
                         $this->user_field);

        $res = $this->dbh->query($queryFlag);        
        $row = $res->fetchRow();
        if($row[0] === 1)
            return;

        $query = sprintf("SELECT %s as prefkey, %s as prefval FROM %s ".
                         "WHERE %s = 'default_pref'",
                         $this->key_field,
                         $this->val_field,
                         $this->table,
                         $this->user_field);
        ...
        while ($row = $res->fetchRow(DB_FETCHMODE_ASSOC)) {
            /*$query = sprintf("insert into webmail_userprefs (%s,\"%s\",\"%s\") values ('$username','{$row['prefkey']}','{$row['prefval']}') ",
                            $this->user_field,$this->key_field, $this->val_field);*/

   
            $insert[] = sprintf("INSERT INTO %s (%s, %s, %s) VALUES  ('$username','{$row['prefkey']}','{$row['prefval']}')",
                             $this->table,
                             $this->user_field,
                             $this->key_field,
                             $this->val_field);
        ...
        $insert[] = sprintf("INSERT INTO %s (%s, %s, %s) VALUES  ('$username','updatepref',1)",
                             $this->table,
                             $this->user_field,
                             $this->key_field,
                             $this->val_field);
        foreach($insert as $ins){            
            $res = $this->dbh->simpleQuery($ins);
