     ...
     && (!$check_referrer || empty($referrer)
      || ($check_referrer && !empty($referrer)
       && (strpos(strtolower($referrer), strtolower($plain_check_referrer)) === 0
        ...
        if (defined('PAGE_NAME') && PAGE_NAME == 'signout' or PAGE_NAME == 'form_import') {
           return;
        }
        ...
        if (!$message)
            echo msgErrorPage(_("You must be logged in to access this page."));
        else if ($message == 1)
            echo msgErrorPage(_("Your session has expired, but will be resumed after logging in again."));
        else if ($message == 2)
            echo msgErrorPage(_("The current page request appears to have originated from an unrecognized source."));
        exit;
    }
}
