...
global $AllowSpamFilters,$chosen_theme;
    ...
    if ($complete_post) {
        ...
        if ($action != 'edit') {
			...
			if(count($filters) == 0)
				$id = 0;
			/*else{
				$sql2 = "select max(SUBSTRING(prefkey,7,7)) from userprefs where prefkey like '%filter%'
				and \"user\" = '$username' and length(prefkey) = 7";
				$res = $db->query($sql2);
				while ($res->fetchInto($row)){
					$id = ++$row[0];
				}
			}	*/	
			$select = "select * from userprefs where prefkey = 'filter$theid'";	
			$res = $db->query($select);
			// From,hhhhh,INBOX.Drafts
			if($res->numRows())
				$sql = "update userprefs set prefval = '$filter_where,$filter_what,$filter_folder' where prefkey = 'filter$theid' and \"user\" = '$username'";
			else
				$sql = "insert into userprefs (\"user\",prefkey,prefval) values ('$username','filter$theid','$filter_where,$filter_what,$filter_folder')";                                   
            $res = $db->query($sql);	
        }
