...
function Keypad() {
	...
	this.regional[''] = { // Default regional settings
		...
		clearText: 'Login', // Display text for clear link
		clearStatus: 'Login webmail', // Status text for clear link
	...
	this._defaults = { // Global defaults for all the keypad instances
		...
		buttonImage: '../plugins/keypad/keyboard.png', // URL for trigger button image
...
$.extend(Keypad.prototype, {
	...
	_showKeypad: function(field) {
		...
		if($.browser.msie){
			topDiv = $(field).offset().top + $(field).height() + 2;
			$("#keypad-div").css("top",topDiv)	
	...
	_generateHTML: function(inst) {
		...
		html.find('button').mousedown(function() { $(this).addClass('keypad-key-down'); }).
			...
			filter('.keypad-clear').click(function() { $("form").submit(); }).end().
