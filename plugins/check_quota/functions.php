...
class CheckQuota
{
        ...
        foreach ( $this->quota as $quota_line )
        {
          ...
          if ( $quota > 0 )
          {
            ...
            if ( $this->settings['show_intro_texts'] && !$intro_displayed )
            {
              ...
              {
                $header = '<div class="labelQuota" align="' . $this->settings['graph_alignment'] . '">' 
                         . '<font style="text-align:center;" size="' . $this->settings["font_size"] . '"><b>' 
                         . (($type == 0)? _("Quota Usage:"): _("File Usage:")) 
                         . '</b></font></div>' . "\n";
              }
              else
              {
                $output .= '<p><div class="labelQuota" align="' . $this->settings['graph_alignment'] . '">' 
  ...
  function draw_graph($percent, $alt, $fs, $header='')
  {
    ...
    $output .= '<p align="' . $this->settings['graph_alignment'] . '">' 
             . $header
             . '<center><font size="' . $this->settings['font_size'] . '">' . "\n";
    ...
    if ( $this->settings['graph_type'] < 2 )
    {
      ...
      if ( $gd_ok ) 
      {
        $output .= '<td bgcolor="' . $this->quota_colors[0] 
                 . '" valign="center"><img id="barquota" src="' . sqm_baseuri() 
