...
/* Carrega para cada usuário as preferências de banco*/
...
$load_listmail = getPref($data_dir, $username, 'load_listmail', SMPREF_ON );
$msg_with_checkbox= getPref($data_dir, $username, 'msg_with_checkbox', SMPREF_OFF);
$alt_index_colors =
...
$javascript_on = 1; //getPref($data_dir, $username, 'javascript_on', SMPREF_ON);
...
$collapse_folder_INBOX = getPref($data_dir, $username, 'collapse_folder_INBOX', 0);
$layout = getPref($data_dir, $username, 'layout', 'horizontal');
...
/*Popula a preferência inicial dos usuários*/
/*if($msg_with_checkbox == "")
    setPref($data_dir, $username, 'msg_with_checkbox', getPref($data_dir, 'default_pref', 'layout'));
if($collapse_folder == "")
    setPref($data_dir, $username, 'collapse_folder', getPref($data_dir, 'default_pref', 'collapse_folder'));
if($collapse_folder_INBOX == "")
    setPref($data_dir, $username, 'collapse_folder_INBOX', 0);
if($sort == "")
    setPref($data_dir, $username, 'sort', 6);
if($chosen_theme == "")
    setPref($data_dir, $username, 'chosen_theme', '../themes/alien_glow.php');
if($layout == "")
    setPref($data_dir, $username, 'layout', 'horizontal');
if($order1 == "")
    setPref($data_dir, $username, 'order1', 1);
if($order2 == "")
    setPref($data_dir, $username, 'order2', 5);
if($order3 == "")
    setPref($data_dir, $username, 'order3', 2);
if($order4 == "")
    setPref($data_dir, $username, 'order4', 4);
if($order5 == "")
    setPref($data_dir, $username, 'order5', 3);
if($order6 == "")
    setPref($data_dir, $username, 'order6', 6);
if($show_num == "")
    setPref($data_dir, $username, 'show_num', 30);
if($html_mail_aggressive_reply_with_unsafe_images == "")
    setPref($data_dir, $username, 'html_mail_aggressive_reply_with_unsafe_images', 1);
if($html_mail_aggressive_reply == "")
    setPref($data_dir, $username, 'html_mail_aggressive_reply', 1);
if($compose_window_type == "")
    setPref($data_dir, $username, 'compose_window_type', 'html');
if($attachment_common_show_images == "")
    setPref($data_dir, $username, 'attachment_common_show_images', 1);
if($alt_index_colors == "")
    setPref($data_dir, $username, 'alt_index_colors', 0);
if($use_signature == "")
    setPref($data_dir, $username, 'use_signature', 1);
if($prefix_sig == "")
    setPref($data_dir, $username, 'prefix_sig', 1);
if($load_listmail == "")
    setPref($data_dir, $username, 'load_listmail', 1);
if($select_msg_with_checkbox == "")
    setPref($data_dir, $username, 'select_msg_with_checkbox', 1);
*/
