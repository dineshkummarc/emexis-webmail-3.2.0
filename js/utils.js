...
//Defina qual o loader será usado na requisção
var loader = "#loader";
...
function contextMenuTrash(){
     $(".left .folder[name=INBOX.Trash]").contextMenu({
            ...
            switch(action){
                case "open":
                    loadListMails($("#alignWebmail").val(),el.attr("name"));
                    break;
                case "empty":
                    $.ajax({
                        url: 'empty_trash.php',
    ...
    $(".gridmail td").contextMenu({
            ...
            switch(action){
                ...
                case "forward":
                    ...
                    break;
                case "delete":
                    if($('.gridmail input[type=checkbox]:checked').size() > 0){
...
function dropMsg(){
    $(".folder").droppable({
        ...
        drop: function(event, ui) {
            $(".gridmail input[type=checkbox]:checked").parent().siblings().removeClass("dragMsg");
            $(".gridmail input[type=checkbox]:checked").each(
                function(index){
                    mailId[index] = $(this).val();
                }
            );
            moveMsg(mail[0].smtoken,mail[0].mailbox,arrNameFolder[$(".folder").index(this)],'Mover',mailId,mail[0].startMessage,mail[0].locate);
            $(".gridmail input[type=checkbox]:checked").parent().siblings().removeClass("dragMsg");
...
function buildDrag(){
    ...
    $(".move div").draggable({
        ...
        start:function(e){
            ...
            $(".gridmail input[type=checkbox]:checked").parent().siblings().addClass("dragMsg");
            if($("#alignWebmail").val() == 'vertical')
                $(".gridmail input[type=checkbox]:checked").parent().parent().next().find('.tdSubject').addClass("dragMsg");
            $(".gridmail input[type=checkbox]:checked").parent().addClass("dragMsg");
            $("#boxAjuda ul").empty();
            $(".gridmail input[type=checkbox]:checked").each(
                function(){
                    if($('#alignWebmail').val() == 'vertical')
                        $("<li style='margin: 0;' >" + $(this).parent().parent().next().find(".textSubject").text() + "&nbsp;&nbsp;</li>").appendTo("#boxAjuda ul");
                    else
        ...
        stop: function(){ ;
            ...
            if($("#alignWebmail").val() == 'vertical')
                $(".gridmail input[type=checkbox]:checked").parent().parent().next().find('.tdSubject').removeClass("dragMsg");
            $(".gridmail input[type=checkbox]:checked").parent().siblings().removeClass("dragMsg");
            $(".gridmail input[type=checkbox]:checked").parent().removeClass("dragMsg");

            if($(".highlight").size() == 0)
                $('.gridmail input:checkbox').attr("checked",false);
        ...
        success: function(dados) {
            ...
            $('.folder').each(
                function(index){
                    $(this).click(
                        function(){
                            loadListMails($("#alignWebmail").val(),$(this).attr("name"));
                            verifyUnread();
                        }
                    );
                }
...
function preloadAjax(align){
    ...
    $(loader).ajaxStop(function() {
       ...
       // $("body").css("background-color","silver");
        if(!$(".left").is(":visible")){
            $('.left,.header,.right,.barcontrol,#divisorLeft,#divisorVertical').show();
        ...
        resizeMail(align);
        /* CheckBox com plugin de switch como o do iphone*/
        /*if($(".dialogOptions").is(":visible") && $('.switchArea').size() == 0){
            $(".dialogOptions input[type='checkbox']").checkToggle();
        }*/
    });
...
function resizeMail(align){
    ...
    if(align == "horizontal"){
        ...
        if($.browser.msie){
            $('.readVertical,.listmailVertical').height($(window).height() - $(".header").height() - 20);
            correctBarIE();
            width = $(window).width() - $('.listmailVertical').width() - $(".left").width() - 65;
            $('.readVertical').width(width);
        }else{
            $('.readVertical,.listmailVertical').height(window.innerHeight - $(".header").height() - 20);
            width = window.innerWidth - $('.listmailVertical').width() - $(".left").width() - 65;
            $('.readVertical').width(width);
        }
        return;
    }else{
        if($.browser.msie){
            $('.listmailWindow').height($(window).height() - $(".header").height());
        }else{
            $('.listmailWindow').height(window.innerHeight - $(".header").height());
        }
...
function openLink(link,ele){ //dialogFolders
    ...
    if($(ele).parent().attr('class') == 'contact' || $(ele).parent().attr('class') == 'liOpenContact'
...
function constructForm(id){
    ...
    $('input[name=attach]').click(function() {
       if($('.compose input[type=text]:focus')){
           return;
       }
...
function openCompose(passed_id,mailbox,startMessage,passed_ent_id,smaction){
    ...
    dialog = '<div id="' + id +'"title="Compose"><div></div></div>';
    $(dialog).dialog({
        stack:true,
        height: 470,
    ...
    if(passed_id == null && mailbox == null && startMessage == null && passed_ent_id == null && smaction == null){
         $.ajax({
            ...
            success: function(dados) {
                ...
                $(".btnAttach,.btnAttachEn,#cancelAttach").click(
                   function(){
                      $("#inputAttach").toggle();
                   }
                );
    ...
    $.ajax({
        ...
        success: function(dados) {
            if($('input[name=flagSearch]').val() == 'true'){
                ...
                loadListMails('horizontal',mailbox,0,0);
            }
            verifyUnread(mailbox);
        }
    });
     ...
     $(".checkAll").toggle(
        function(){
            $(".gridmail input[type=checkbox]")
                ...
                .addClass('highlight');
                if(align == 'vertical')
                    $(".gridmail input[type=checkbox]").parent().parent().next().children().addClass('highlight');
            $(".gridmail input[type=checkbox]")
                .parent()
                .addClass('highlight');
        },
        function(){
            ...
            $(".gridmail input[type=checkbox]")
                ...
                .removeClass('highlight');
            if(align == 'vertical')
                    $(".gridmail input[type=checkbox]").parent().parent().next().children().removeClass('highlight');
        }
     );
}
...
function popupNotification(subject,from,id){
    if($('#not' + id).size() == 0 ){
        ...
        var notice = '<div id="not' + id + '" class="notice">'
                ...
                + '<h3>' + from + '</h3>'
                + '<p style="cursor:pointer !important" onclick=loadMailWin("INBOX",' + id + ',"' + $("input:hidden[name=startMessage]").val() + '","")>' + subject + '</p>'
                + '</div>'
...
function improvementDialog(){
    $('.ui-widget-header').append("<a role='button' class='ui-dialog-titlebar-maxi ui-corner-all' href='#'><span class='ui-icon ui-icon-extlink maximize'></span></a>");
    $('.ui-dialog-titlebar-maxi').hover( function() {$(this).addClass('ui-state-hover');} , function() {$(this).removeClass('ui-state-hover');});
    $('.ui-dialog-titlebar-maxi').focus( function() {$(this).addClass('ui-state-focus');} , function() {$(this).removeClass('ui-state-focus');});

    $('.ui-widget-header').append("<a role='button' class='ui-dialog-titlebar-min ui-corner-all' href='#'><span class='ui-icon ui-icon-minus'></span></a>");
    $('.ui-dialog-titlebar-min').hover( function() {$(this).addClass('ui-state-hover');} , function() {$(this).removeClass('ui-state-hover');});
    $('.ui-dialog-titlebar-min').focus( function() {$(this).addClass('ui-state-focus');} , function() {$(this).removeClass('ui-state-focus');})
...
function maximizeWindow(){
     ...
     //alert($("#contentOptions").size());
     /*if($("#contentOptions").is(":e")){
        alert(1)
        $('#contentOptions').height($('#dialogOptions').height() - 100);
        return;
    }*/
     $('.maximize').toggle(
        function(){
...
function resizeCompose(id){
    ...
    // $(".cke_editor").height($(id).parent().height() - 300);
    // $(".cke_editor iframe").height($(id).parent().height() - 300);

    /*$(".cke_editor iframe").height($(id).parent().height() - 200);
    $(".cke_editor").height($(id).parent().height() - 300);    */
                 ...
                 function(e){
                    if(e.keyCode == 13){
                       if($(this).val() <  parseInt($("#TotalPages").text()) && $(this).val() > 0){
                          ...
                          loadListMails($("#alignWebmail").val(),mailbox,0,0,1,'', msg);
                       }else{
                           return;
                       }
                    }
...
function inputSearch(){
    ...
    $('#search input:text').keypress(
        function(e){
            if(e.keyCode == 13){
                search($("input[name=smtoken]").val(),$("input[name=mailbox]").val(),$("input[name=what]").val(),$("input[name=where]:checked").val());
            }
        }
    );
    ...
    $.ajax({
        ...
        success: function(dados) {
            ...
            if(str.search('Successfully Saved Options') != -1){
                ...
                setTimeout("$('.saveError').slideUp('slow')",4000);
            }
...
function editContact(){
    ...
    email = $('.ContInfo .addremail').val();
    firstname = $('.ContInfo .addrfirstname').val();
    lastname = $('.ContInfo .addrlastname').val();
    label = $('.ContInfo .addrlabel').val();

    str = '&editaddr[nickname]=' + nickname + '&editaddr[email]=' + email + '&editaddr[firstname]=' + firstname
...
function addContact(){
    ...
    str = '&addaddr[nickname]=' + nickname + '&addaddr[email]=' + email + '&addaddr[firstname]=' + firstname
    ...
    $(".btnRenameGroup").click(
        function(){
            $.ajax({
                ...
                type: 'GET',
                data: 'action=4&group=' + $("#selectGroup]").val() + '&newName=' + $('input[type=text]').val(),
                dataType: 'text',
                success:function(dados){
...
function reloadWebmail(){
    loadFolders(true);
    loadListMails($("#alignWebmail").val(),"INBOX");
}
